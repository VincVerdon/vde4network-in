...
void printlog_file(int priority, const char *format, ...)
{
	...
	if (f != NULL) {
		...
		fprintf(f, "%s : ", severity_text[priority]);
		fprintf(f, format);
		fprintf(f, "\n");
		fclose(f);
	}
...
// VV 20260217 add logfile and savefile
...
static void usage(void)
{
	...
	printf(
			...
			"  -l, --logfile              Log file name. If not defined log system is used\n"
			"  -p, --pidfile PIDFILE      Write pid of daemon to PIDFILE\n"
			"  -w, --savefile             Default configuration save file name\n"
